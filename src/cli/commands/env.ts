...
import { formatDotenv } from "../../core/dotenv.js"
...
const buildDotenvHeader = (configPath: string): string =>
  `# Generated by envpkt — regenerate with: envpkt env dotenv\n# Source: ${configPath}. Do not edit by hand.`

const runEnvDotenv = (options: DotenvOptions): void => {
  resolveForEmit(options).fold(
    (err) => {
      console.error(formatError(err))
      process.exit(2)
    ...
    (boot) => {
      ...
      // EmitEntry ({ name, value, secret }) is structurally a DotenvEntry.
      const entries = collectEmitEntries(boot)
      const includeSecrets = options.secrets !== false
      const text = formatDotenv(entries, { includeSecrets, header: buildDotenvHeader(boot.configPath) })
      ...
      Try(() => writeFileSync(outPath, `${text}\n`, "utf-8")).fold(
        ...
        () => {
          ...
          if (includeSecrets && entries.some((e) => e.secret)) {
            console.error(`${YELLOW}Note:${RESET} ${outPath} contains secret values — ensure it is .gitignored.`)
          }
        },
      )
    },
...
export const registerEnvCommands = (program: Command): void => {
  ...
  env
    .command("dotenv")
    .description("Output resolved credentials in .env format (for Wrangler, Docker --env-file, Vite, etc.)")
    .option("-c, --config <path>", "Path to envpkt.toml")
    .option("--profile <profile>", "fnox profile to use")
    .option("-o, --output <file>", "Write to a file instead of stdout")
    .option("--no-secrets", "Omit secret values (emit KEY= with a note) — values are included by default")
    .action((options: DotenvOptions) => {
