---
title: Full Agent
description: Complete envpkt.toml with agent identity, lifecycle, callbacks, and secrets
---

A comprehensive configuration demonstrating all available sections.

```toml
#:schema https://raw.githubusercontent.com/jordanburke/envpkt/main/schemas/envpkt.schema.json

# Full envpkt configuration for an AI agent with lifecycle policies

version = 1

[identity]
name = "data-pipeline-agent"
consumer = "agent"
description = "ETL pipeline processor for data ingestion"
capabilities = ["read-s3", "write-postgres", "send-notifications"]
expires = "2027-01-01"
services = ["aws", "postgres", "slack"]

# --- Lifecycle Policy ---

[lifecycle]
stale_warning_days = 90
require_expiration = true
require_service = true

# --- Callbacks ---

[callbacks]
on_expiring = "https://hooks.slack.com/services/T00/B00/xxx"
on_expired = "https://hooks.slack.com/services/T00/B00/yyy"
on_audit_fail = "https://hooks.slack.com/services/T00/B00/zzz"

# --- Tool Integrations ---

[tools]
fnox = true
mcp = true

# --- Environment Defaults (non-secret, safe to commit) ---

[env.NODE_ENV]
value = "production"
purpose = "Runtime environment mode"
comment = "Override to 'development' for local testing"

[env.LOG_LEVEL]
value = "info"
purpose = "Application log verbosity"

# --- Secret Metadata ---

[secret.AWS_ACCESS_KEY_ID]
service = "aws"
purpose = "S3 bucket read access for raw data ingestion"
capabilities = ["s3:GetObject", "s3:ListBucket"]
created = "2025-11-01"
expires = "2026-05-01"
rotation_url = "https://console.aws.amazon.com/iam/home#/security_credentials"
source = "vault"

[secret.AWS_SECRET_ACCESS_KEY]
service = "aws"
purpose = "S3 bucket read access (secret counterpart)"
capabilities = ["s3:GetObject", "s3:ListBucket"]
created = "2025-11-01"
expires = "2026-05-01"
rotation_url = "https://console.aws.amazon.com/iam/home#/security_credentials"
source = "vault"

[secret.DATABASE_URL]
service = "postgres"
purpose = "Write processed data to the analytics database"
comment = "Shared with the reporting service — coordinate rotation"
capabilities = ["SELECT", "INSERT", "UPDATE"]
created = "2025-10-15"
expires = "2026-10-15"
rotation_url = "https://wiki.internal/runbooks/rotate-db-creds"
source = "vault"

[secret.SLACK_WEBHOOK_URL]
service = "slack"
purpose = "Send pipeline completion and failure notifications"
capabilities = ["post-message"]
created = "2025-09-01"
rotation_url = "https://api.slack.com/apps/A00/incoming-webhooks"
source = "manual"

# Alias: expose the same governed webhook under a legacy name that the
# notifier library still expects. Both names are injected at boot.
[secret.LEGACY_SLACK_HOOK]
from_key = "secret.SLACK_WEBHOOK_URL"
purpose = "Legacy env var name read by the older notifier client"
```

## Sections Breakdown

- **`[identity]`** — identifies the consumer, its role, capabilities, and service dependencies
- **`[lifecycle]`** — configures audit policies (stale warnings, required fields)
- **`[callbacks]`** — Slack webhooks triggered on lifecycle events
- **`[tools]`** — enables fnox and MCP integrations
- **`[env.*]`** — plaintext environment defaults (non-secret, safe to commit)
- **`[secret.*]`** — per-secret metadata with service, purpose, comment, capabilities, dates, and source
- **`[secret.*].from_key`** — [alias](/reference/toml-schema/#aliases) to another entry; the value is inherited from the target and injected under both canonical names
