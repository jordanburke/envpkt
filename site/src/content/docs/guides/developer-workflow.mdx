...
```
env scan → catalog.toml → cloud-synced folder → ENVPKT_CONFIG → eval "$(envpkt env export)"
```

1. **Discover** what's in your environment with `env scan`
2. **Catalog** the secrets with metadata (service, expiration, rotation)
3. **Sync** the catalog via iCloud Drive, OneDrive, or a private git repo
4. **Pick up** secrets per-terminal with `env export` + shell hook
