...
This creates `[secret.*]` sections for each secret defined in fnox, pre-populating the `service` field where possible.
...
envpkt supports encrypted identity via age keys:
...
The `key_file` field points to an age identity (private key) file. The `recipient` field is the agent's public key for encryption.
