...
envpkt is pre-1.0, so the moving **major tag is `v0`** — it's re-pointed to each `0.x` release
by the publish workflow, so `uses: jordanburke/envpkt@v0` always resolves to the newest `0.x`.

- **`@v0`** — moving major; picks up new `0.x` releases automatically. Recommended for most.
- **`@v0.13.4`** — exact release; immutable, opt into upgrades deliberately.
- **`@v1`** — not published yet; it ships when envpkt reaches `1.0.0`.

Note this is the **action** version. The envpkt **CLI** version it runs is separate — controlled
by the `version` input (default `latest`); pin it for reproducible builds.
...
which is **not** preinstalled on GitHub-hosted runners and has **no bundled/JS fallback** — it's
a hard requirement for unsealing. Install it before the `envpkt` step:
...
**Gotcha:** because those plaintext defaults and fnox values resolve _without_ `age`, a run can
look healthy while every sealed secret silently fails to decrypt. If sealed values aren't showing
up in later steps, confirm `age` is installed — don't assume the packets are the problem.
:::
